...
module.exports = (() => {
	...
	class AdapterForHttp extends AdapterBase {
		constructor(host, port, secure, onAlertCreated, onAlertMutated, onAlertDeleted, onAlertTriggered) {
			...
			this._restProvider = new RestProvider(this.host, this.port, this.secure);
			...
			this._updateAlertsForUserEndpoint = new RestEndpoint(RestAction.Update, [ 'alerts', 'users', 'alert_system', 'user_id' ]);
			...
			this._retrieveModifiersEndpoint = new RestEndpoint(RestAction.Retrieve, [ 'alert', 'modifiers' ]);
			...
			this._userEndpoint = new RestEndpoint(RestAction.Retrieve, [ 'user' ]);
