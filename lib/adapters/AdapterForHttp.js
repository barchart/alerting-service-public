...
module.exports = (() => {
	...
	class AdapterForHttp extends AdapterBase {
		constructor(host, port, secure, onAlertCreated, onAlertMutated, onAlertDeleted, onAlertTriggered) {
			...
			this._retrieveAlertTriggerStatusesEndpoint = EndpointBuilder.for('retrieve-alert-trigger-statuses', 'Retrieve alert trigger statuses')
				.withVerb(VerbType.GET)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
				.withPathBuilder((pb) => {
					pb.withLiteralParameter('alerts', 'alerts')
						.withLiteralParameter('trigger', 'trigger')
						.withLiteralParameter('status', 'status')
				})
				.withQueryBuilder((pb) => {
					pb.withVariableParameter('alert_system', 'alert_system', 'alert_system')
						.withVariableParameter('user_id', 'user_id', 'user_id')
						.withVariableParameter('trigger_date', 'trigger_date', 'trigger_date', true)
			...
			this._updateAlertTriggerStatusEndpoint = EndpointBuilder.for('update-alert-trigger-status', 'Update alert trigger status')
				.withVerb(VerbType.PUT)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
			...
			this._updateAlertTriggerStatusesEndpoint = EndpointBuilder.for('update-alert-trigger-statuses', 'Update alert trigger statuses')
				.withVerb(VerbType.PUT)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
				.withPathBuilder((pb) => {
					pb.withLiteralParameter('alerts', 'alerts')
						.withLiteralParameter('trigger', 'trigger')
						.withLiteralParameter('statuses', 'statuses')
				})
				.withBody()
				.withRequestInterceptor(requestInterceptor)
				.withResponseInterceptor(ResponseInterceptor.DATA)
