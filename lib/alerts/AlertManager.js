	...
	var AlertManager = Disposable.extend({
		...
		createAlert: function(alert) {
			...
			for (var i = 0; i < alert.conditions.length; i++) {
				...
				if (operandQualifiers) {
					for (var j = 0; j < operandQualifiers.length; j++) {
						var operandQualifier = operandQualifiers[j];

						assert.argumentIsRequired(operandQualifier, 'alert.conditions[' + i + '].operator.operand_qualifiers[' + j + ']', String);
					}
				}
			}
