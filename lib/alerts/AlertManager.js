...
module.exports = (() => {
	...
	class AlertManager extends Disposable {
					...
					if (this._connectPromise === null) {
						const alertAdapterPromise = Promise.resolve()
							.then(() => {
								...
								if (mode === 'rest') {
									adapter = new RestAlertAdapter(this._host, this._port, this._secure, onAlertCreated.bind(this), onAlertMutated.bind(this), onAlertDeleted.bind(this));
								} else if (mode === 'socket.io') {
									adapter = new SocketAlertAdapter(this._host, this._port, this._secure, onAlertCreated.bind(this), onAlertMutated.bind(this), onAlertDeleted.bind(this));
								} else {
						...
						const instrumentManagerPromise = Promise.resolve()
							.then(() => {
								const host = 'instruments-stage.aws.barchart.com';
								const secure = this._secure;
								const port = getPort(this._secure);

								const manager = new InstrumentManager(host, port, 'rest', secure);

								return timeout(manager.connect(), 10000, 'Alert service is temporarily unavailable. Please try again later.');
							});

						this._connectPromise = Promise.all([alertAdapterPromise, instrumentManagerPromise])
							.then((results) => {
								this._adapter = results[0];
								this._instrumentManager = results[1];
		...
		checkSymbol(symbol) {
			return Promise.resolve()
				.then(() => {
					...
					return this._instrumentManager.lookupInstrument(symbol);
				}).then((result) => {
					...
					const instrumentMap = alert.conditions.reduce((map, c) => {
						...
						if (property.target.type === 'symbol') {
							...
							if (!map.hasOwnProperty(symbol)) {
								map[symbol] = this._instrumentManager.lookupInstrument(symbol);
							}
						}
							...
							validatePromise = instrumentMap[symbol]
								.then((result) => {
									...
									if (property.format === 'price' && operator.operand_type === 'number' && operator.operand_literal) {
										...
										const price = priceParser(operandToParse, converter.baseCodeToUnitCode(instrument.unitcode), ',');
		...
		subscribeAlerts(query, changeCallback, deleteCallback, createCallback) {
			...
			const createRegistration = subscriptionData.createEvent.register(changeCallback);
