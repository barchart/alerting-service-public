...
module.exports = (() => {
	...
	class AlertManager extends Disposable {
		...
		async createAlert(alert) {
			...
			const [ properties, operators ] = await Promise.all([
				this.getProperties(),
				this.getOperators()
			]);
		...
		async enableAlerts(query) {
			...
			await this._adapter.updateAlertsForUser({ user_id: query.user_id, alert_system: query.alert_system, alert_state: 'Starting' });
		...
		async disableAlerts(query) {
			...
			await this._adapter.updateAlertsForUser({user_id: query.user_id, alert_system: query.alert_system, alert_state: 'Stopping'});
		...
		static async filterPropertiesForSymbol(properties, symbol, target, alertSystem) {
			...
			const { instrument } = await lookupInstrument(symbol, alertSystem || null);
