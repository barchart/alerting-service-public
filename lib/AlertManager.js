...
const AdapterBase = require('./adapters/AdapterBase'),
	AdapterForHttp = require('./adapters/AdapterForHttp'),
	AdapterForSocketIo = require('./adapters/AdapterForSocketIo'),
...
module.exports = (() => {
	...
	class AlertManager extends Disposable {
		...
		/**
		 * Creates and starts a new {@link AlertManager} with HTTP adapter for use in the private staging environment.
		 *
		 * @public
		 * @static
		 ...
		 * @returns {Promise<AlertManager>}
		 */
		static forStagingHttp(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.stagingHost, DEFAULT_SECURE_PORT, true, AdapterForHttp), jwtProvider);
				});
		}

		/**
		 * Creates and starts a new {@link AlertManager} with Socket.IO adapter for use in the private staging environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forStagingSocketIO(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.stagingHost, DEFAULT_SECURE_PORT, true, AdapterForSocketIo), jwtProvider);
				});
		}

		/**
		 * Creates and starts a new {@link AlertManager} with HTTP adapter for use in the private production environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forProductionHttp(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.productionHost, DEFAULT_SECURE_PORT, true, AdapterForHttp), jwtProvider);
				});
		}

		/**
		 * Creates and starts a new {@link AlertManager} with Socket.IO adapter for use in the private production environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forProductionSocketIO(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.productionHost, DEFAULT_SECURE_PORT, true, AdapterForSocketIo), jwtProvider);
				});
		}

		/**
		 * Creates and starts a new {@link AlertManager} with HTTP adapter for use in the private admin environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forAdminHttp(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.adminHost, DEFAULT_SECURE_PORT, true, AdapterForHttp), jwtProvider);
				});
		}

		/**
		 * Creates and starts a new {@link AlertManager} with Socket.IO adapter for use in the private admin environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forAdminSocketIO(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.adminHost, DEFAULT_SECURE_PORT, true, AdapterForSocketIo), jwtProvider);
				});
		}


		/**
		 * Creates and starts a new {@link AlertManager} with HTTP adapter for use in the private demo environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forDemoHttp(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.demoHost, DEFAULT_SECURE_PORT, true, AdapterForHttp), jwtProvider);
				});
		}

		/**
		 * Creates and starts a new {@link AlertManager} with Socket.IO adapter for use in the private demo environment.
		 *
		 * @public
		 * @static
		 * @param {JwtProvider} jwtProvider
		 * @returns {Promise<AlertManager>}
		 */
		static forDemoSocketIO(jwtProvider) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(jwtProvider, 'jwtProvider', JwtProvider, 'JwtProvider');

					return start(new AlertManager(Configuration.demoHost, DEFAULT_SECURE_PORT, true, AdapterForSocketIo), jwtProvider);
				});
		}
